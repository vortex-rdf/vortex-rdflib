...
A Python port of the generator in vortex-rdf's ``js/bench/shared.ts``, minus
named graphs (rdflib stores are compared triple-wise here). Distinct terms
scale with rows: near-unique subject IRIs (~10 triples per subject), a small
closed predicate vocabulary, and a mix of IRI and literal objects, so each
store's term handling (dictionaries, interning, string storage) is actually
exercised.
...
def moduli(cfg: DatasetConfig) -> Moduli:
    ...
    # IRIs, so they are not new terms.
    reused = sum(1 for j in range(min(n_subj, n_obj)) if not _is_literal_index(j, cfg))
    return Moduli(n_subj, n_pred, n_obj, n_subj + n_pred + n_obj - reused)
