...
graph patterns only), for bisecting and A/B measurements.
"""
...
class Relation:
    ...
    predicates still to be applied while a columnar body streams.
    """
...
def _check_block(node) -> None:
    if getattr(node, "name", None) not in _BLOCK_NODES:
        raise NotImplementedError
...
def _solve_bgp(ctx, store, triples) -> Relation:
...
def _rel_from_pattern(pat) -> Relation:
    ...
    The zero-copy views are kept as the body; nothing is copied into Python
    objects until rows are consumed. Repeated variables (``?x :p ?x``) and
    ground patterns go through ``_materialize`` instead.
    """
    varpos = pat["varpos"]
    ...
    return Relation(tuple(varpos), cols, None, pat["nrows"])
...
def _materialize(pat):
    ...
    surviving variable keeps its first position. An empty match still names
    its variables, so the operators above can pad or project by schema.
    """
    varpos = pat["varpos"]
    schema = tuple(varpos)
    if pat["nrows"] == 0:
    ...
    if eq_checks:
        keep = [
            i for i in range(pat["nrows"]) if all(views[a][i] == views[b][i] for a, b in eq_checks)
        ]
