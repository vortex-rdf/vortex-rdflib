...
`OPTIONAL`, `MINUS`, `FILTER (NOT) EXISTS`, nested groups and inline
`VALUES` tables over patterns run as hash left joins, anti-joins, semi-joins
and joins over the same code tuples (an inner pattern is re-probed per outer
row when the outer side is small), instead of rdflib re-entering the store
once per outer solution. The
